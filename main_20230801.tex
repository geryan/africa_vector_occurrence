...
\author[1,3,4]{\fnm{Nicholas} \sur{Golding}}\email{nickgoldingresearch@gmail.com}
...
\abstract{
\textbf{Background:} 
Spatial modelling of malaria vectors useful.
Bias in locations where mosquitoes collected.
Can cause bias in model outputs and therefore impinge on malaria vector control.
Methods exist in the ecological modelling literature to account for this and produce unbiased outputs.\\

\textbf{Methods:} 
Simulation study to demonstrate scale of the problem.
Building a model of bias using data on reported detections, and the locations of malaria vector research centres in Africa.
exploration/illustration of issue in existing data set (MAP vector occurrence set)\\

\textbf{Results:} 
Bias is a big issue, leading to incorrect predictions that vectors are distributed closer to urban areas.
We have developed a model of bias which can be used to correct models for this bias.
This map also shows areas with poor surveillance for malaria vectors which could be targeted to improve our understanding of malaria risk.\\

\textbf{Conclusion:} 
There are legitimate reasons why data collection is biased in certain areas - e.g. data are collected for many purposes. However when modelling data from various sources it is important to understand sources of bias and account for these in analyses, and where possible collect and report absence data along with presences. \\
}
